...
assert(state.project.version === 7.6, "Project schema version must be 7.6.");
...
assert(warmedPresets.every((preset) => (preset.filter1?.cutoff || 0) <= 7200), "One or more factory presets retain an overly bright primary filter.");
...
assert(warmedPianos.every((preset) => (preset.softTone?.presenceDipDb || 0) <= -3), "Warm piano presets are missing strong brittle-presence control.");
...
assert(engine.includes("maxVoicesPerTrack = 20"), "Adaptive per-track voice limit is missing.");
assert(engine.includes("maxVoicesTotal = 72"), "Adaptive global voice limit is missing.");
...
if (errors.length) {
  console.error("DAW 7.6 validation failed:");
  errors.forEach((error) => console.error(` - ${error}`));
  process.exit(1);
}

console.log("DAW 7.6 validation passed: warm piano/preset voicing, click-free pause flushing, processing-graph reset, Spacebar restart-from-zero transport, phase-aware envelopes, adaptive voice-cost polyphony, indexed realtime/offline note scheduling, phase-aligned multiband crossovers, parallel-FX headroom, oversampled realtime ceiling, linked true-peak offline protection, Engine Guard controls, and  plugin-based track/master processing, room reverb, tempo delay, Time Shaper, multiband compression, stereo imaging, pitch shifting, production EQ, stable Synth Lab rendering, timbre/pitch modulation, per-note pan and stereo width, plus optimized memoized row rendering, indexed note hit detection, GPU note positioning, animation-frame drag throttling, a single playhead/scale-guide layer, per-note ADSR and gain overrides for synth and sampler tracks, realtime/offline envelope scheduling, plus all DAW 7.1 sampling, track, synth, mastering, and piano-roll workflows.");

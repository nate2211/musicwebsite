...
const browserCompatSource = read("src/studio/audio/browserAudioCompat.js");
const schedulerSource = read("src/studio/audio/precisionScheduler.js");
const sampleLibraryCompatSource = read("src/studio/state/sampleLibrary.js");
...
assert(multitrackPlannerSource.includes("Second pass reserves one voice per remaining active track"), "Fair per-track voice reservation is missing.");
...
assert(browserCompatSource.includes("createCompatibleAudioContext"), "Chrome/Firefox AudioContext constructor fallback is missing.");
assert(browserCompatSource.includes("resumeCompatibleAudioContext"), "Cross-browser AudioContext resume handling is missing.");
assert(browserCompatSource.includes("unlockCompatibleAudioContext"), "Cross-browser first-gesture output unlock is missing.");
assert(browserCompatSource.includes("decodeAudioDataCompat"), "Promise/callback audio decoding compatibility is missing.");
assert(browserCompatSource.includes("createStereoPannerCompat"), "Stereo panner compatibility fallback is missing.");
assert(browserCompatSource.includes("installAudioResumeRecovery"), "Visibility and user-gesture audio recovery is missing.");
assert(engine.includes("compatibleBaseLatency"), "Browser-aware latency floor is missing.");
assert(engine.includes("browserProfile.schedulerIntervalMs") || engine.includes("browserProfile?.schedulerIntervalMs"), "Browser-specific scheduler interval is missing.");
assert(effectsSource.includes("createStereoPannerCompat"), "Effects rack does not use cross-browser stereo panning.");
assert(voicesSource.includes("createStereoPannerCompat"), "Voice engine does not use cross-browser stereo panning.");
assert(offline.includes("decodeAudioDataCompat"), "Offline renderer does not use cross-browser decoding.");
assert(sampleLibraryCompatSource.includes("decodeAudioDataCompat"), "Imported samples do not use cross-browser decoding.");
assert(schedulerSource.includes("try {\n        worker = new Worker"), "Worker scheduler does not fall back when browser worker creation is blocked.");
...
assert(engine.includes("transportGain.gain.setTargetAtTime"), "Pause does not fade the dedicated transport output before graph reset.");
...
assert(engine.includes("masterTrackFromProject(project)"), "Realtime master rack does not consume project master settings and volume.");
assert(offline.includes("masterTrackFromProject(project)"), "Offline rendering does not use the master plugin rack and master volume.");
...
assert(state.project.version === 8.8, "Project schema version must be 8.8.");
...
try {
  ...
  const multitrackCandidates = ["piano", "bass", "drums", "pad"].flatMap((trackId, trackIndex) => (
    Array.from({ length: 24 }, (_, noteIndex) => ({
      ...
      trackId,
      role: trackId === "drums" ? "drums" : trackId === "bass" ? "bass" : trackId === "piano" ? "melody" : "harmony",
  ...
  const multitrackPlan = planner.planMultitrackStep(multitrackCandidates, {
    ...
    perRoleVoiceBudget: 12,
    perRoleCostBudget: 90,
  });
  ...
  const roleCounts = multitrackPlan.reduce((map, job) => map.set(job.role, (map.get(job.role) || 0) + 1), new Map());
  assert([...roleCounts.values()].every((count) => count <= 12), "Multitrack planner exceeded a musical-role voice budget.");
...
assert(engine.includes("createNativeMixBuses"), "Isolated native mix buses are missing.");
assert(engine.includes("updateNativeMixBuses"), "Native mix buses are not updated per project.");
assert(engine.includes("perRoleVoiceBudget"), "Per-role voice budgeting is missing.");
assert(multitrackPlannerSource.includes("First pass reserves one voice per active musical role"), "Musical-role reservation is missing.");
assert(multitrackPlannerSource.includes("perRoleCostBudget"), "Per-role node-cost budgeting is missing.");
assert(engine.includes("native Web Audio"), "Native Web Audio is not the default realtime route.");

assert(engine.includes("scheduleAudibleOutputCheck"), "First-play audible output verification is missing.");
assert(engine.includes("activateDirectOutputFallback"), "Silent processed-master fallback is missing.");
assert(engine.includes("activateTrackInputFallback"), "Silent track-processing fallback is missing.");
assert(engine.includes("masterBypassGain"), "Native master bypass path is missing.");
assert(engine.includes("compatBypass"), "Per-track native bypass path is missing.");
assert(engine.includes("resetAudio(project"), "User-triggered native audio reset is missing.");
assert(engine.includes("processed master produced no audible signal"), "Processed-master audible fallback reason is missing.");
const compat = await import("../src/studio/audio/browserAudioCompat.js");
const chromeProfile = compat.detectAudioBrowser("Mozilla/5.0 Chrome/128.0.0.0 Safari/537.36");
const firefoxProfile = compat.detectAudioBrowser("Mozilla/5.0 Firefox/129.0");
assert(chromeProfile.chrome && chromeProfile.chromium, "Google Chrome user-agent detection failed.");
assert(firefoxProfile.firefox && !firefoxProfile.chromium, "Mozilla Firefox user-agent detection failed.");
assert(firefoxProfile.minimumLookAheadSeconds > chromeProfile.minimumLookAheadSeconds, "Firefox scheduler safety window is not larger than Chrome's.");
const promiseDecoded = await compat.decodeAudioDataCompat({ decodeAudioData: async (bytes) => ({ length: bytes.byteLength }) }, new ArrayBuffer(12));
assert(promiseDecoded.length === 12, "Promise-based Chrome-style decodeAudioData compatibility failed.");
const callbackDecoded = await compat.decodeAudioDataCompat({ decodeAudioData: (bytes, success) => setTimeout(() => success({ length: bytes.byteLength }), 0) }, new ArrayBuffer(13));
assert(callbackDecoded.length === 13, "Callback-based decodeAudioData compatibility failed.");

if (errors.length) {
  console.error("DAW 8.8 validation failed:");
  errors.forEach((error) => console.error(` - ${error}`));
  process.exit(1);
}

console.log("DAW 8.8 validation passed: Google Chrome and Mozilla Firefox browser profiles, first-gesture AudioContext creation/resume, Promise and callback audio decoding, stereo-panner fallback, blocked-worker scheduler fallback, browser-aware latency windows, visibility recovery, isolated multitrack scheduling, dense-note admission, plugin safety, protected rendering, GPU piano-roll output, 500 factory WAV assets, and 240 synthesizer presets.");
process.exit(0);

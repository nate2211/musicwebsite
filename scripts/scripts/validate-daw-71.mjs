...
assert(state.project.version === 7.1, "Project schema version must be 7.1.");
...
if (errors.length) {
  console.error("DAW 7.1 validation failed:");
  errors.forEach((error) => console.error(` - ${error}`));
  process.exit(1);
}

console.log("DAW 7.1 validation passed: softer, warmer and lighter factory presets, gentle presence shaping, persistent local audio and drum imports, preset uploads, waveform slicing, loop rasterization, slice-pattern generation, plus  draggable accessible rotary knobs, Shift fine adjustment, persistent realtime/offline master-volume slider, direct synth-track creation, multi-track management, 240 layered enterprise presets, and all DAW 6.7 piano-roll workflows.");

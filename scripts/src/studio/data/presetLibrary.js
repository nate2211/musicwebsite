...
export function warmDigitalPreset(rawPreset) {
  ...
  const baseProfile = SOFT_WARM_CATEGORY_PROFILE[patch.category] || SOFT_WARM_CATEGORY_PROFILE.default;
  const isPiano = /piano|tines|keys/i.test(`${rawPreset?.name || ""} ${rawPreset?.tags?.join(" ") || ""}`);
  const airyCategory = ["Keys", "Pluck", "Bell", "Flute", "Arp", "Choir", "World"].includes(patch.category);
  const profile = {
    ...baseProfile,
    brightness: Math.min(0.48, baseProfile.brightness * 0.98 + 0.018),
    cutoff: Math.min(8200, baseProfile.cutoff * 1.035 + 120),
    airCutoff: Math.min(15800, Math.max(11200, baseProfile.airCutoff + (airyCategory ? 3200 : 2400))),
    presenceDip: Math.min(-1.1, baseProfile.presenceDip + 0.3),
    warmth: baseProfile.warmth + 0.2,
    clarity: airyCategory ? 0.78 : 0.58,
    airShelf: airyCategory ? 0.72 : 0.46,
    outputTrim: baseProfile.outputTrim * 0.93,
    ...(isPiano ? {
      brightness: 0.36,
      cutoff: 6500,
      attack: 0.012,
  ...
  const softenOscillator = (oscillator, index) => ({
    ...
    level: Math.min(
      isPiano ? (index === 0 ? 0.58 : index === 1 ? 0.22 : 0.1) : (index === 0 ? 0.64 : index === 1 ? 0.3 : 0.2),
      oscillator.level ?? 0,
    ),
    fine: Math.max(isPiano ? -7 : -10, Math.min(isPiano ? 7 : 10, oscillator.fine || 0)),
  });
  const softenLayer = (layer, index) => ({
    ...layer,
    level: Math.min(isPiano ? 0.09 : 0.14, layer.level ?? 0),
    detune: Math.min(isPiano ? 7 : 10, layer.detune ?? 6),
    stereo: Math.min(isPiano ? 0.42 : 0.54, layer.stereo ?? 0.35),
  ...
  return {
    ...
    tags: [...new Set([...(rawPreset.tags || []), "warm", "digital", "soft", "light", "smooth", "airy", "production"])],
    description: `${rawPreset.description || "Original factory instrument."} Production Air voicing keeps the patch warm and soft while adding controlled top-end openness, low-mid separation, and mix-ready clarity.`,
    ...
    sub: { ...patch.sub, level: Math.min(isPiano ? 0.1 : 0.18, patch.sub?.level || 0) },
    ...
    filter1: {
      ...
      resonance: Math.min(isPiano ? 0.82 : 1.25, patch.filter1?.resonance || 0.7),
      drive: Math.min(isPiano ? 0.025 : 0.055, Math.max(0.006, patch.filter1?.drive || 0)),
    },
    filter2: {
      ...patch.filter2,
      cutoff: Math.min(Math.max(180, profile.cutoff * 0.72), patch.filter2?.cutoff || profile.cutoff),
      resonance: Math.min(isPiano ? 0.7 : 1, patch.filter2?.resonance || 0.5),
      drive: Math.min(isPiano ? 0.018 : 0.035, Math.max(0, patch.filter2?.drive || 0)),
    },
    filterEnv: {
      ...patch.filterEnv,
    ...
    fm: { ...patch.fm, amount: Math.min(isPiano ? 0.025 : 0.055, patch.fm?.amount || 0) },
    ring: { ...patch.ring, amount: Math.min(isPiano ? 0.018 : 0.04, patch.ring?.amount || 0) },
    lfo1: softenLfo(patch.lfo1),
    lfo2: softenLfo(patch.lfo2),
    voiceFx: {
      ...patch.voiceFx,
      bitcrush: Math.min(isPiano ? 0 : 0.006, patch.voiceFx?.bitcrush || 0),
      saturation: Math.min(isPiano ? 0.035 : 0.065, Math.max(0.012, patch.voiceFx?.saturation || 0)),
      chorus: Math.min(isPiano ? 0.14 : 0.24, patch.voiceFx?.chorus || 0),
      chorusDepth: Math.min(isPiano ? 0.0028 : 0.004, patch.voiceFx?.chorusDepth || 0.003),
    },
    macros: {
      ...patch.macros,
    ...
    unison: Math.min(isPiano ? 3 : 5, patch.unison || 1),
    detune: Math.min(isPiano ? 7 : 10, patch.detune || 0),
    stereo: Math.min(isPiano ? 0.46 : 0.6, patch.stereo ?? 0.35),
    voiceDrift: Math.min(isPiano ? 1.5 : 2.5, patch.voiceDrift || 0),
    softTone: {
      lowCut: profile.lowCut,
      warmthDb: profile.warmth,
      mudFrequency: isPiano ? 315 : 360,
      mudDipDb: isPiano ? -1.35 : -0.9,
      clarityFrequency: isPiano ? 1650 : 1850,
      clarityDb: profile.clarity,
      presenceFrequency: isPiano ? 3050 : 3250,
      presenceDipDb: profile.presenceDip,
      velvetFrequency: isPiano ? 6100 : 6600,
      velvetDb: isPiano ? -0.42 : -0.28,
      airShelfDb: profile.airShelf,

    ...
    const draw = (now = 0) => {
      ...
      if (analyser) {
        ...
        if (now - lastStatusAt > 500) {
          ...
          const rate = stream?.sampleRate ? `${Math.round(stream.sampleRate / 100) / 10} kHz` : "rate pending";
          const latency = Number.isFinite(stream?.latencyMs) ? `${stream.latencyMs} ms` : "latency pending";
          setStatus(`${stream?.browser || "Web Audio browser"} · ${stream?.contextState || "not-started"} · ${rate} · ${latency} · ${stream?.peakDb ?? "−∞"} dB · ${stream?.load ?? 0}% load`);
        }
      }

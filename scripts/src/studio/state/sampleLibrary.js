import { createCompatibleAudioContext, decodeAudioDataCompat } from "../audio/browserAudioCompat";
...
async function decodeMetadata(blob) {
  if (!(globalThis.AudioContext || globalThis.webkitAudioContext)) return { duration: 0, channels: 0, sampleRate: 0 };
  const context = createCompatibleAudioContext({ latencyHint: "playback" });
  try {
    const buffer = await decodeAudioDataCompat(context, await blob.arrayBuffer());
    return {

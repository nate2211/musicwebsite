...
export default function StudioPage({ initialView }) {
  ...
  useEffect(() => {
    ...
    window.__MUSICSTUDIOLAB_AUDIO__ = {
      getStatus: () => engineRef.current?.getStreamStatus?.(),
      reset: () => engineRef.current?.resetAudio?.(stateRef.current.project),
    };
    return () => {
      try { delete window.__MUSICSTUDIOLAB_AUDIO__; } catch (_) { window.__MUSICSTUDIOLAB_AUDIO__ = undefined; }
    };
  }, []);

...
assert(presetLibrarySource.includes("bitcrush: Math.min(0.035"), "Factory patches are not protected from harsh bit-crushing.");
...
assert(state.project.version === 7.0, "Project schema version must be 7.0.");
...
assert(warmedPresets.every((preset) => preset.warmDigital), "Factory presets are not marked with warm digital voicing.");
assert(warmedPresets.every((preset) => (preset.voiceFx?.bitcrush || 0) <= 0.035), "One or more factory presets remain excessively bit-crushed.");
assert(warmedPresets.every((preset) => (preset.filter1?.resonance || 0) <= 3.2), "One or more factory presets retain harsh filter resonance.");
...
if (errors.length) {
  console.error("DAW 7.0 validation failed:");
  errors.forEach((error) => console.error(` - ${error}`));
  process.exit(1);
}

console.log("DAW 7.0 validation passed: warm digital presets, persistent local audio and drum imports, preset uploads, waveform slicing, loop rasterization, slice-pattern generation, plus  draggable accessible rotary knobs, Shift fine adjustment, persistent realtime/offline master-volume slider, direct synth-track creation, multi-track management, 240 layered enterprise presets, and all DAW 6.7 piano-roll workflows.");

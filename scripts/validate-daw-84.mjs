...
// Verify the optional Elementary insert uses one stereo Web Audio input bus.
...
const fakeSource = {
  connect: (target) => { connectedTarget = target; return target; },
  disconnect: () => {},
};
...
assert(initializeOptions?.numberOfInputs === 1, "Elementary runtime must expose one stereo input bus.");
assert(JSON.stringify(initializeOptions?.inputChannelCount) === JSON.stringify([2]), "Elementary input bus must contain two channels.");
assert(stereoProbe.connectInput(fakeSource), "Elementary stereo input connection failed.");
assert(connectedTarget === fakeNode, "Stereo master stream was not connected directly to the Elementary input bus.");
stereoProbe.disconnect();
assert(engine.includes("ElementaryRealtimeEngine"), "Optional realtime Elementary engine is missing.");
assert(engine.includes('realtimeEngineMode = "native"'), "Protected native output is not the default realtime engine.");
assert(engine.includes("configureRealtimeBackend"), "Realtime output backend selection is missing.");
assert(engine.includes('requested = project?.master?.realtimeEngine === "elementary"'), "Elementary must be explicitly selected before realtime activation.");
...
assert(elementaryRealtimeSource.includes("numberOfInputs: 1"), "Elementary runtime is not configured with one stereo input bus.");
assert(elementaryRealtimeSource.includes("inputChannelCount: [2]"), "Elementary input bus is not configured for stereo.");
assert(elementaryRealtimeSource.includes("returnAnalyser.getFloatTimeDomainData"), "Elementary takeover is not verified from its real post-render return.");
assert(engine.includes("nativeFallbackGain"), "Protected native output gain is missing.");
assert(engine.includes("activateElementaryStream"), "Optional Elementary output cannot be activated after return verification.");
assert(engine.includes("activateNativeStreamFallback"), "Elementary failures do not preserve native audio output.");
assert(pluginRackSource.includes('label="Realtime output"'), "Engine Guard realtime output selector is missing.");
assert(pluginRackSource.includes('value: "native"'), "Protected native output is not offered in Engine Guard.");
...
assert(state.project.version === 8.4, "Project schema version must be 8.4.");

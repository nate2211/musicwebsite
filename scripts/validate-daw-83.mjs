...
// Verify that the browser renderer receives two native input channels and that
// the native stereo stream is explicitly split into left/right worklet inputs.
const fakeNodeConnections = [];
const fakeNode = {
  channelCount: 0,
  channelCountMode: "",
  channelInterpretation: "",
  connect: (destination) => { fakeNodeConnections.push(destination); return destination; },
  disconnect: () => {},
};
let initializeOptions = null;
class FakeRenderer {
  constructor() { this.handlers = new Map(); }
  async initialize(_context, options) { initializeOptions = options; return fakeNode; }
  on(name, handler) { this.handlers.set(name, handler); }
  async render(...roots) { return { roots: roots.length, elapsedTimeMs: 0 }; }
  async reset() {}
}
const splitterConnections = [];
const fakeSplitter = {
  connect: (node, output, input) => splitterConnections.push({ node, output, input }),
  disconnect: () => {},
};
const fakeGainParam = {
  value: 0,
...
const fakeContext = {
  currentTime: 0,
  createGain: () => fakeOutputGain,
  createChannelSplitter: (channels) => {
    assert(channels === 2, "Elementary stereo input splitter is not two-channel.");
    return fakeSplitter;
  },
};
const fakeSource = {
  connect: (target) => { assert(target === fakeSplitter, "Master stream was not connected to the stereo splitter."); },
  disconnect: () => {},
};
const stereoProbe = new ElementaryRealtimeEngine({ RendererClass: FakeRenderer });
await stereoProbe.initialize(fakeContext, { id: "destination" });
assert(initializeOptions?.numberOfInputs === 2, "Elementary runtime is not configured with two native input channels.");
assert(JSON.stringify(initializeOptions?.inputChannelCount) === JSON.stringify([1, 1]), "Elementary worklet inputs are not explicit mono left/right channels.");
assert(stereoProbe.connectInput(fakeSource), "Elementary stereo input connection failed.");
assert(splitterConnections.length === 2, "Elementary stereo splitter did not connect both channels.");
assert(splitterConnections.some((entry) => entry.output === 0 && entry.input === 0), "Left channel was not mapped to Elementary input 0.");
assert(splitterConnections.some((entry) => entry.output === 1 && entry.input === 1), "Right channel was not mapped to Elementary input 1.");
stereoProbe.disconnect();
assert(engine.includes("ElementaryRealtimeEngine"), "Realtime engine is not connected to Elementary Audio.");
assert(engine.includes("Elementary Audio shared stereo DSP + protected output"), "Elementary shared stereo DSP backend status is missing.");
...
assert(elementaryRealtimeSource.includes("numberOfInputs: 2"), "Elementary runtime is not configured for stereo native input.");
assert(elementaryRealtimeSource.includes("inputChannelCount: [1, 1]"), "Elementary left/right worklet inputs are not explicit.");
assert(elementaryRealtimeSource.includes("createChannelSplitter(2)"), "Elementary master input is not split into deterministic stereo channels.");
assert(engine.includes("nativeFallbackGain"), "Sound-first native fallback gain is missing.");
assert(engine.includes("activateElementaryStream"), "Elementary output is not activated after signal confirmation.");
assert(elementaryRealtimeSource.includes("this.onSignal?."), "Elementary meter does not confirm an audible return before crossfade.");
...
assert(state.project.version === 8.3, "Project schema version must be 8.3.");

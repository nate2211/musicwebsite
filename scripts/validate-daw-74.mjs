...
assert(state.project.version === 7.4, "Project schema version must be 7.4.");
...
assert(audioSafetySource.includes("attackEnd = Math.min(noteEnd"), "Long attacks are not clamped to the available note gate.");
...
assert(engine.includes("maxVoicesPerTrack = 24"), "Per-track voice limit is missing.");
assert(engine.includes("maxVoicesTotal = 96"), "Global voice limit is missing.");
...
assert(safety.highNoteHeadroom(120) > 0.6, "High-note headroom is muting the upper register too aggressively.");

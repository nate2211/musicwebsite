...
assert(piano.includes("scale-ghost-note"), "Selected-scale ghost-note markers are missing.");
...
try {
  ...
  assert(markup.includes("scale-ghost-note"), "Compiled piano roll did not render selected-scale ghost notes.");
  assert(markup.includes("Ghost tones: C · D · D# · F · G · G# · A#"), "Compiled C minor ghost-tone legend is incorrect.");
} finally {
  await vite.close();
}

if (errors.length) {
  console.error("DAW 6.5 validation failed:");
  errors.forEach((error) => console.error(` - ${error}`));
  process.exit(1);
}

console.log("DAW 6.5 validation passed: left-draw/right-erase piano-roll gestures, selected-scale ghost notes, selected-track mounting, sampler note playback, sample-derived titles, and 444 factory WAV assets.");

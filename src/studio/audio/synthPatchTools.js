...
export function mutatePatch(input, intensity = 0.25) {
  ...
  return {
    ...
    layers: patch.layers.map((layer) => ({
      ...mutateOsc(layer),
      enabled: Math.random() < intensity * 0.25 ? !layer.enabled : layer.enabled,
      unison: Math.round(jitter(layer.unison || 1, 4 * intensity, 1, 9)),
      detune: jitter(layer.detune || 0, 22 * intensity, 0, 50),
      stereo: jitter(layer.stereo || 0, 0.55 * intensity, 0, 1),
      motion: jitter(layer.motion || 0, 0.45 * intensity, 0, 1),
      motionRate: jitter(layer.motionRate || 0.1, 0.8 * intensity, 0.01, 4),
    })),
    textureLayer: {
      ...patch.textureLayer,
      enabled: Math.random() < intensity * 0.22 ? !patch.textureLayer.enabled : patch.textureLayer.enabled,
      level: jitter(patch.textureLayer.level || 0, 0.12 * intensity, 0, 0.35),
      motion: jitter(patch.textureLayer.motion || 0, 0.45 * intensity, 0, 1),
      motionRate: jitter(patch.textureLayer.motionRate || 0.1, 0.8 * intensity, 0.01, 4),
    },
    ampEnv: {
      ...patch.ampEnv,
...
export function randomizePatch() {
  ...
  return {
    ...
    layers: patch.layers.map((layer, index) => ({
      ...layer,
      enabled: Math.random() > (index === 0 ? 0.28 : 0.48),
      waveform: randomWave(),
      octave: [-1, 0, 0, 1, 2][Math.floor(Math.random() * 5)],
      semitone: [0, 0, 3, 5, 7, 12][Math.floor(Math.random() * 6)],
      fine: Math.round((Math.random() * 2 - 1) * 14),
      level: 0.06 + Math.random() * 0.24,
      pan: index === 0 ? -0.18 : 0.18,
      unison: 2 + Math.floor(Math.random() * 7),
      detune: 5 + Math.random() * 35,
      stereo: 0.25 + Math.random() * 0.75,
      delay: Math.random() * 0.06,
      motion: Math.random() * 0.6,
      motionRate: 0.03 + Math.random() * 1.2,
    })),
    textureLayer: {
      ...patch.textureLayer,
      enabled: Math.random() > 0.45,
      color: ["white", "pink", "brown", "blue"][Math.floor(Math.random() * 4)],
      level: Math.random() * 0.12,
      highpass: 40 + Math.random() * 1600,
      lowpass: 3000 + Math.random() * 15000,
      resonance: Math.random() * 4,
      stereo: 0.35 + Math.random() * 0.65,
      motion: Math.random() * 0.7,
      motionRate: 0.03 + Math.random() * 0.8,
    },
    ampEnv: {
      ...patch.ampEnv,
...
export function interpolatePatch(aInput, bInput, amount) {
  ...
  return {
    ...
    layers: a.layers.map((layer, index) => interpolateObject(layer, b.layers[index] || layer)),
    textureLayer: interpolateObject(a.textureLayer, b.textureLayer),

...
export class AudioEngine {
  constructor(onStep) {
    ...
    this.streamBackend = "Direct native Web Audio";
  ...
  configurePerformance(project) {
    ...
    const deviceFloor = Math.max(0, Number(this.context?.baseLatency || 0)) * 3 + 0.04;
    this.lookAhead = Math.max(0.09, Math.min(0.24, Math.max(configuredLookAhead, deviceFloor)));
  }
  ...
  getStreamStatus() {
    ...
    return { backend: this.streamBackend, peakDb, load, buses: Object.fromEntries(this.busCounts || []) };
  }

  async unlockOutputGraph() {
    if (!this.context || this.context.state !== "running") return false;
    if (this.outputUnlocked) return true;
    const source = this.context.createBufferSource();
    const silentGain = this.context.createGain();
    source.buffer = this.context.createBuffer(1, 1, this.context.sampleRate);
    silentGain.gain.setValueAtTime(0.000001, this.context.currentTime);
    source.connect(silentGain).connect(this.streamInput || this.context.destination);
    source.start(this.context.currentTime);
    source.stop(this.context.currentTime + 0.002);
    source.onended = () => {
  ...
  async ensure({ unlock = true } = {}) {
    ...
    // The default realtime route is deliberately direct native Web Audio.
    // AudioWorklet loading is not allowed to delay or replace audible output.
